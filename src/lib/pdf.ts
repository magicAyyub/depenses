...
export function exportToPDF(expenseMonth: ExpenseMonth): void {
  ...
  doc.text(`Total: ${formattedTotal} F CFA`, pageWidth / 2, currentY, { align: 'center' });
  currentY += 25;
  ...
  expenseMonth.expenses.forEach((expense: OldExpense) => {
    ...
    doc.text(`${formattedAmount} F CFA`, currentX, currentY);
  ...
  doc.text(`TOTAL: ${formattedFinalTotal} F CFA`, pageWidth - margin - 80, currentY);
...
export function exportAllExpensesToPDF(expenseMonths: ExpenseMonth[]): void {
  ...
  doc.text(`Total Général: ${formattedTotalGeneral} F CFA`, pageWidth / 2, currentY, { align: 'center' });
  currentY += 25;
  ...
  expenseMonths.forEach((month) => {
    ...
    doc.text(`${monthName}: ${formattedMonthTotal} F CFA (${month.expenses.length} dépenses)`, margin + 10, currentY);
    currentY += 8;
  });
